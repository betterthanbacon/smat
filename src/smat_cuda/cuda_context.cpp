...
public:
	...
	{
		// First, call cudaMalloc/Free on a tiny allocation, as a way to 
		// wait until the cuda runtime is "ready" for calls to cuMemGetInfo
		void* ptr = 0; 
		cudaMalloc(&ptr,32);
		cudaFree(ptr);

		// cuGetMemInfo tells us what we need.
		size_t free = 0, total = 0;
		CUresult meminfo_status = cuMemGetInfo(&free,&total);
		SM_ASSERT(meminfo_status == CUDA_SUCCESS);
		return free;
	}
};
